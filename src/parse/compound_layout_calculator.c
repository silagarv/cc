...
typedef struct LayoutCalculator {
    ...
    size_t size; // The final size of the compound type
    size_t alignment; // the final alignmnt of the compound type

    size_t bitfield_offset; // the current bit-offset for the bitfield
...
static void calculate_field_layout(LayoutCalculator* state, Declaration* member)
{
    ...
    size_t type_size = !flex_array ? qualified_type_get_size(&real_type) : 0;
    size_t type_align = qualified_type_get_align(&real_type);
    ...
    else if (declaration_field_has_bitfield(member))
    {
        ...
        size_t bitfield_size = declaration_field_get_bitfield(member);
        ...
        else
        {
            // printf("offset is: %zu, and %zu bits\n", state->size,
            //     state->bitfield_offset);
            size_t post_offset = state->bitfield_offset + bitfield_size;
